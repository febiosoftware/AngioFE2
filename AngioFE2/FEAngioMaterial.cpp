...
mat3ds FEAngioMaterial::SproutStress(const vec3d& y)
{
	...
	double den_scale = findDenScale(y.x, y.y, y.z);
...
//-----------------------------------------------------------------------------
// Finds the density scale factor.
double FEAngioMaterial::findDenScale(double xpt, double ypt, double zpt)
{
	// Make sure we have a grid defined
	if (m_pgrid == 0) return 1.0;

	// Find the element where this point lives in
	int elem_num = m_pgrid->findelem(xpt, ypt, zpt);
    if (elem_num < 0) return 1.0;
	
	// get the element
	Elem& elem = m_pgrid->ebin[elem_num];

    // Convert to natural coordinates -1 <= Xi <= +1
    double xix, xiy, xiz;
    m_pgrid->natcoord(xix, xiy, xiz, xpt, ypt, zpt, elem_num);        
    
    // Obtain shape function weights
    double shapeF[8];
    m_pgrid->shapefunctions(shapeF, xix, xiy, xiz);
    
	// calculate the collagen density at this point
	double coll_den = 
	...
	// evaluate the density scale factor
	double den_scale = m_pgrid->find_density_scale(coll_den);

	// Make sure it's positive
    if (den_scale < 0.) den_scale = 0.;
